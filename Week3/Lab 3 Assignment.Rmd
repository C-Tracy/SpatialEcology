...
means <- c(mean(as.matrix(simpRastI2)), mean(as.matrix(simpRastI5)), mean(as.matrix(simpRastI10)))
variances <- c(var(as.matrix(simpRastI2)), var(as.matrix(simpRastI5)), var(as.matrix(simpRastI10)))
grain <- c(2, 5, 10)
...
Based on AICc values, the 2km scale is the best model (lowest AIC) to explain the data given the models used. The Delta AIC indicates that the models for 2-5km are all decent models (all have a delta AIC lower than 2), but overall the 2 km scale model is the best supported model of the models used. 
